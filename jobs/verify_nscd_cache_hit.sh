...
echo 'sed --in-place -e "/^.*debug-level/s/0/1/" -e "/^#.*reload-count.*[0-9]$/s/[0-9]*$/0/;/^#.*reload-count.*0$/s/#//" -e "/^#.*logfile.*log$/s/#//" /etc/nscd.conf' | docker exec -i docker-client-1 su -
...
echo 'uname -a' | docker exec -i docker-client-1 su -
...
echo 'cd /glibc-build && /usr/local/src/glibc-2.31/configure --prefix=/glibc-2.31' | docker exec -i docker-client-1 su -
