...
echo 'wget https://ftp.gnu.org/gnu/glibc/glibc-2.31.tar.gz' | docker exec -i docker-client-1 su -
echo 'tar -C /usr/local/src -x -f glibc-2.31.tar.gz' | docker exec -i docker-client-1 su -
echo 'mkdir /glibc-build && cd /glibc-build && /usr/local/src/configure --prefix=/glibc-2.31' | docker exec -i docker-client-1 su -
